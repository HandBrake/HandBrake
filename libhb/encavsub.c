...
hb_encavsub_context_t * encavsubInit( hb_work_object_t * w, hb_job_t * job )
{
    ...
    context->subtitle_header = av_malloc(ctx->subtitle->extradata_size);
    if (context->subtitle_header == NULL)
    {
        hb_error("encavsubInit: av_malloc subtitle_header failed");
        goto fail;
    }
    memcpy(context->subtitle_header, ctx->subtitle->extradata,
                                     ctx->subtitle->extradata_size);
    context->subtitle_header_size  = ctx->subtitle->extradata_size;
    ...
    ctx->subtitle->extradata = NULL;

    if (context->extradata != NULL && context->extradata_size > 0)
    {
        uint8_t * tmp = malloc(context->extradata_size);
        if (tmp == NULL)
        {
            hb_error("encavsubInit: malloc subtitle extradata failed");
            goto fail;
        }
        memcpy(tmp, context->extradata, context->extradata_size);
        ctx->subtitle->extradata = tmp;
        ctx->subtitle->extradata_size = context->extradata_size;
    }
                  ...
                  hb_buffer_t ** buf_out )
{
    ...
    subtitle.rects = av_mallocz_array(num_rects, sizeof(subtitle.rects));
