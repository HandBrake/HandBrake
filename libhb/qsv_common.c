...
static int hb_qsv_param_parse_gpu_index(hb_job_t *job, const mfxAdaptersInfo* adapters_info, const int gpu_index)
{
    for (int i = 0; i < adapters_info->NumActual; i++)
    {
        ...
        // if -1 use adapter with highest priority
        if (info && ((info->Number == gpu_index) || (gpu_index == -1)))
        {
            ...
            hb_log("hb_qsv_param_parse: %s qsv adapter with index %s has been selected",
...
int hb_create_ffmpeg_pool(hb_job_t *job, int coded_width, int coded_height, enum AVPixelFormat sw_pix_fmt, int pool_size, int extra_hw_frames, AVBufferRef **out_hw_frames_ctx)
{
    ...
    if (!job->qsv.ctx->hb_hw_device_ctx) {
        ...
        if (job->encoder_options != NULL && *job->encoder_options)
        {
                ...
                iter  = hb_dict_iter_next(options_list, iter))
            {
                ...
                if (!strcasecmp(key, "gpu"))
                {
                    ...
                    int gpu_index = hb_qsv_atoi(str, &ret);
                    if (!ret)
                    {
                        ret = hb_qsv_param_parse_gpu_index(job, &qsv_adapters_info, gpu_index);
                        if (ret == 0)
                        {
                            gpu_option_present = 1;
                        }
                    }
                    free(str);
                }
        ...
        if (!gpu_option_present)
            hb_qsv_param_parse_gpu_index(job, &qsv_adapters_info, -1);
