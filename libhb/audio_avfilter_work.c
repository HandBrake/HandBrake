...
static int audio_avfilter_init(hb_work_object_t * w, hb_job_t * job)
{
    ...
    pv->filter_string = hb_audio_filter_list_build_string(
                            pv->audio->config.out.audio_filters);
    const char * filter_settings = pv->filter_string;
    ...
    if (graph_out_channels != pv->in_channels)
    {
        ...
        else
        {
            ...
            hb_log("audio_avfilter_init: filter output %d channels "
                   "(unsupported by encoder), constraining to input layout",
                   graph_out_channels);
            hb_log("audio_avfilter_init: WARNING: track %d: audio filter "
                   "wants %d-channel output but the selected encoder "
                   "does not support it. Channel layout change will be "
                   "reverted. Consider using a different encoder "
                   "(e.g. eac3, opus, flac).",
                   pv->audio->config.out.track, graph_out_channels);
