...
static int     image_sequence      = 0;
static char *  sequence_framerate  = NULL;
...
int main( int argc, char ** argv )
{
    ...
    else
    {
        ...
        hb_scan(h, input, titleindex, image_sequence, sequence_framerate, preview_count, store_previews,
...
fprintf(stderr, "+++++ Framerate Test: %s\n", hb_value_get_string(hb_dict_get(job_dict, "SequenceFramerate")));
...
static int ParseOptions( int argc, char ** argv )
{
    ...
    for( ;; )
    {
        static struct option long_options[] =
          {
            ...
            { "image-sequence", required_argument,     NULL, IMAGE_SEQUENCE },
        ...
        switch( c )
        {
            ...
            case IMAGE_SEQUENCE:
                image_sequence = 1;
                sequence_framerate = strdup( optarg );
                break;
...
static hb_dict_t * PreparePreset(const char *preset_name)
{
    ...
    if (image_sequence != -1)
    {
        hb_dict_set(preset, "ImageSequence", hb_value_bool(image_sequence));
        hb_dict_set(preset, "SequenceFramerate",
                    hb_value_string(sequence_framerate));
    }
    if (align_av_start != -1)
    {
